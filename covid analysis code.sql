...
-- ( another possible option is to delete, or insert the mean value (to keep the original distribution as is) )
...
SELECT DATEPART(YEAR, Date) AS 'Year', COUNT(DISTINCT(MONTH(Date))) AS 'Month' FROM dbo.Data
...
SELECT DATEPART(YEAR, Date) AS 'Year', DATEPART(MONTH, Date) AS 'Month', COUNT(*) AS 'Number of rows'
FROM dbo.Data
GROUP BY DATEPART(YEAR, Date), DATEPART(MONTH, Date)
ORDER BY 1,2,3


-- min value: confirmed, deaths, recovered per month
...
-- max value: confirmed, deaths, recovered per month
...
-- February 2020 is the month which had the largest number of confirmed cases


-----------------------------------------------------------------------------------------------


-- The dispersion: refers to the spread of the values around the central tendency:
...
-- PERCENTITLES AND FREQUENCY

--Percentiles : Population divided into 100 equal groups
--Percentiles help us to understand the distribution of the data by grouping values into equal sized buckets
...
-- Calculate a simple moving average for the "Confirmed" cases over a 7-day window:


SELECT
    Date,
    Confirmed,
    AVG(Confirmed) OVER (ORDER BY Date ROWS BETWEEN 6 PRECEDING AND CURRENT ROW) AS MovingAvgConfirmed
FROM
    dbo.Data
ORDER BY
    Date;
...
-- Identify data points with "Confirmed" counts that are more than 2 standard deviations away from the mean:
...
SELECT
    ...
    Confirmed,
    CASE
        WHEN Confirmed IS NOT NULL
            AND ABS(Confirmed - AVG(Confirmed) OVER ()) / STDEV(Confirmed) OVER () > 2
        THEN 'Outlier'
        ELSE 'Not Outlier'
    END AS OutlierFlag
FROM
...
-- Perform a t-test to compare the mean "Confirmed" cases between two provinces (Province A and Province B):



SELECT
    'Province A' AS Province,
...
-- Calculate the distance between two locations (latitude and longitude) using the Haversine formula:
...
SELECT
    @radius * 2 * ASIN(SQRT(POWER(SIN(RADIANS(@lat2 - @lat1) / 2), 2) +
    COS(RADIANS(@lat1)) * COS(RADIANS(@lat2)) * POWER(SIN(RADIANS(@lon2 - @lon1) / 2), 2))) AS DistanceInKm;



-- Time Series Decomposition:

-- Decompose the "Confirmed" cases into trend, seasonal, and residual components using a time series library or function specific to your database system.

-- Create a temporary table to store the time series data

CREATE TEMPORARY TABLE TimeSeriesData AS
SELECT
    Date,
    Confirmed
FROM
...
-- Calculate the rolling average (trend component)

CREATE TEMPORARY TABLE Trend AS
SELECT
    Date,
    Confirmed,
    AVG(Confirmed) OVER (
        ORDER BY Date
        ROWS BETWEEN 7 PRECEDING AND 7 FOLLOWING -- Adjust the window size as needed
    ) AS RollingAverage
FROM
    TimeSeriesData;

-- Calculate the seasonal component (remove trend from the original data)

CREATE TEMPORARY TABLE Seasonal AS
SELECT
    t.Date,
    t.Confirmed - r.RollingAverage AS SeasonalComponent
FROM
    TimeSeriesData t
JOIN
    Trend r ON t.Date = r.Date;

-- Calculate the residual component

CREATE TEMPORARY TABLE Residual AS
SELECT
    Date,
    Confirmed - RollingAverage AS ResidualComponent
FROM
    Trend;

-- Display the components (trend, seasonal, and residual)

SELECT
    t.Date,
    t.Confirmed AS OriginalData,
    t.RollingAverage AS TrendComponent,
    s.SeasonalComponent,
    r.ResidualComponent
FROM
    TimeSeriesData t
JOIN
    Seasonal s ON t.Date = s.Date
JOIN
    Residual r ON t.Date = r.Date;
